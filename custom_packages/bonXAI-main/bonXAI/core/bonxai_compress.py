...
def compute_m(n, target_size, g, num_bins):
    n_prime = num_bins * compress.largest_power_of_four(math.floor(n / num_bins))
...
def bonxai_compress(X, kernel_type, k_params=np.ones(1), g=4, num_bins=32,
    ...
    if target_size == None or target_size == int(math.sqrt(nearest_pow_four)):
        return compress.compresspp_kt(X, kernel_type, k_params, g, num_bins, delta, seed, mean0)
