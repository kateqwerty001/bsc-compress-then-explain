...
from bonXAI.core.preprocessor import Compressor, Preprocessor
...
def test_kernel_thinning(monkeypatch, small_data):
    ...
    indices = np.arange(5)
    monkeypatch.setattr("bonXAI.core.preprocessor.bonxai_compress", lambda *a, **kw: indices)
...
def test_stein_thinning_gaussian(monkeypatch, small_data):
    X, y, model = small_data
    monkeypatch.setattr("bonXAI.core.preprocessor.thin", lambda X, grad, m: np.arange(min(len(X), 4)))
    comp = Compressor(X, y, model)
...
def test_stein_thinning_kde(monkeypatch, small_data):
    X, y, model = small_data
    monkeypatch.setattr("bonXAI.core.preprocessor.thin", lambda X, grad, m: np.arange(3))
    comp = Compressor(X, y, model)
...
def test_stein_thinning_gmm(monkeypatch, small_data):
    ...
    monkeypatch.setattr("bonXAI.core.preprocessor.GaussianMixture", FakeGMM)
    monkeypatch.setattr("bonXAI.core.preprocessor.thin", lambda X, grad, m: np.arange(5))
    comp = Compressor(X, y, model)
...
def test_influence_compression(monkeypatch, small_data):
    ...
    monkeypatch.setattr("bonXAI.core.preprocessor.CgInfluence", FakeInf)
...
def test_arfpy_compression(monkeypatch, small_data):
    ...
    monkeypatch.setattr("bonXAI.core.preprocessor.arf_mod", type("M", (), {"arf": FakeARF}))
    comp = Compressor(X, y, model)
